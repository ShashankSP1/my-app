...
export default function Nav() {
  ...
  ${
    theme === "dark"
      ? "bg-gradient-to-r from-blue-950/80 to-indigo-950/80 border-b border-white/10"
      : "bg-gradient-to-r from-white/80 to-gray-100/70 border-b border-gray-300/30"
  }`}
      >
        <img
          src={
            theme === "dark" ? "/Developer_dark.png" : "/Developer_light.png"
          }
          alt="icon"
          className="w-36 sm:w-48 md:w-60 h-10 sm:h-12 object-contain cursor-pointer transition-transform duration-300 hover:scale-105"
        />
        {/* Desktop Menu */}
      ...
      <div
        className={`fixed right-0 h-full w-3/4 z-40 sm:w-1/2 md:hidden backdrop-blur-lg shadow-2xl transform transition-transform duration-500 ${
          isOpen ? "visible opacity-100 translate-x-0" : "invisible opacity-0 translate-x-full"
        }
        `}
      >
        <ul
